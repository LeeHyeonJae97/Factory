...
[CreateAssetMenu(fileName = "ServiceTeam", menuName = "ScriptableObject/ServiceTeam")]
public class ServiceTeamSO : ScriptableObject
{
    ...
    private CapabilitySO _output;
    private MoneySO _upgradeOffering;
    private CapabilitySO _fame;
    private ChiefBundleSO _chiefBundle;

    public UnityAction<ServiceTeamSO> onValueChanged;
    public UnityAction<float> onProgressValueChanged;

    public void Load(EasyFileSave file, CapabilitySO output, MoneySO upgradeOffering, CapabilitySO fame, ChiefBundleSO chiefBundle)
    {
    ...
    private void UpdateValue()
    {
        ...
        OutputAmount = (int)(int.Parse(new DataTable().Compute(string.Format(_outputAmountFormula, Level), null).ToString()) * (1 + _chiefBundle.Buff));
